...
- **uact** — the native denom of the Akash token. Older docs may say `uakt` — that name is deprecated. Always use `uact` in pricing config.
...
- **Use `uact` in your pricing config.** The denom was renamed from `uakt`. Old configs will work for now but should be migrated.
