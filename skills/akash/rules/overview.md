...
| Deployment Token | ACT — denom `uact` (1 ACT = 1,000,000 uact). Used for SDL pricing, bid prices (the rate providers offer), deployment escrow/deposit, and lease payments. **Separate from AKT.** The bid *deposit* (`bidMinDeposit` / `MsgCreateBid.deposit`, anti-spam collateral) defaults to `uakt` — the provider posts 500000 uakt (0.5 AKT) from its balance; `uact` only via a rare burn-mint fallback. |
