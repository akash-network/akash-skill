...
`uact` is the denom for SDL pricing and lease payment on Akash, and is the practical choice for new deployments. The providers endpoint does **not** expose accepted denominations, but in practice `uact` is what deployments and providers settle on for lease payment (`uakt` is only accepted via a rare burn-mint fallback).
